...
#[derive(Serialize, Deserialize, Clone, Eq, PartialEq)]
pub struct ReportChsum<T = FileId> {
    pub file: T,
    pub limit: u64,
...
#[derive(Serialize, Deserialize, Clone, Eq, PartialEq)]
pub struct Start<T = FileId> {
    pub file: T,
    pub offset: u64,
}

#[derive(Serialize, Deserialize, Clone, Eq, PartialEq)]
pub struct Cancel<T = FileId> {
    pub file: T,
}

#[derive(Serialize, Deserialize, Clone, Eq, PartialEq)]
#[serde(tag = "type")]
pub enum ServerMsg<T = FileId> {
    Progress(Progress<T>),
    Done(Done<T>),
    Error(Error<T>),
    ReqChsum(ReqChsum<T>),
...
#[derive(Serialize, Deserialize, Clone, Eq, PartialEq)]
#[serde(tag = "type")]
pub enum ClientMsg<T = FileId> {
    ReportChsum(ReportChsum<T>),
    Error(Error<T>),
    Cancel(Cancel<T>),
}
...
impl<T> From<&ServerMsg<T>> for warp::ws::Message
where
    T: Serialize,
{
    fn from(value: &ServerMsg<T>) -> Self {
        let msg = serde_json::to_string(value).expect("Failed to serialize server message");
        Self::text(msg)
    }
}

impl<T> From<&ClientMsg<T>> for tokio_tungstenite::tungstenite::Message
where
    T: Serialize,
{
    fn from(value: &ClientMsg<T>) -> Self {
